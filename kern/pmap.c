...
page_init(void)
{
	...
	for (int i = 0; i < npages; ++i){
		if (i == 0 || i == PGNUM(MPENTRY_PADDR) || (npages_basemem <= i && i < npages_basemem + num_pages_io_hole) || (npages_basemem + num_pages_io_hole <= i && i < npages_basemem + num_pages_io_hole + num_kpages)){
			pages[i].pp_ref = 1;
			continue;
    	}
		pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	if (page_free_list == NULL)
		return NULL;
	struct PageInfo* tmppage = page_free_list;
	page_free_list = tmppage->pp_link;
	tmppage->pp_link = NULL;
	if (alloc_flags & ALLOC_ZERO)
		memset(page2kva(tmppage), 0, PGSIZE);
	return tmppage;
}
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_link != NULL || pp->pp_ref != 0)
		panic("page_free: freeing a page which is not allocated");
